import React, { Fragment, useState } from 'react'
import { useLocation, useNavigate } from 'react-router'
import { getCreateOrder, verifyPaymentUser } from '../../../api_Integration/User/users'
import useRazorpay from "react-razorpay";
import { useSelector } from 'react-redux';


function PaymentSection() {
    const navigate = useNavigate()
    const Razorpay = useRazorpay()
    const {user} = useSelector((state) => state.users)
    const {owner} = useSelector((state) => state.owners)

    const [checked,setChecked] = useState(false)
    const [email,setEmail] = useState(user.email)
    const [phone,setPhone] = useState(user.phone)
    const [payment,setPayment] = useState('razorpay')

    const location = useLocation()

    const showDetails = location.state.data
    const showData = showDetails.showData
    const selectedSeats = showDetails.selectedSeats
    const showDate = showDetails.date

    const result = (selectedSeats.length * showData.ticketprice)/20
    const convenienceFee = Math.floor(result)

    const discount = 0;

    const total = (showData.ticketprice * selectedSeats.length) + convenienceFee + discount

    const handleRadio = (checked) => {
        console.log(checked)
        if(checked === false){
            setChecked(true)
        }else{
            setChecked(false)
        }
    }

    const paymentData = {
        user:user,
        owner:owner,
        email : email,
        phone : phone,
        price : total,
        showData : showData,
        selectedSeats : selectedSeats,
        showDate : showDate
    }

    const handleSubmit = async(e) => {
        if(checked===false){
            alert('select a payment method')
        }
        e.preventDefault()
        const response = await getCreateOrder({amount:total})
        console.log(response)
        const order = response

        var options = {
            "key": process.env.Razorpay_KEY_ID, // Enter the Key ID generated from the Dashboard
            "amount": order.amount, // Amount is in currency subunits. Default currency is INR. Hence, 50000 refers to 50000 paise
            "currency": "INR",
            "name": "Cineawe",
            "description": "Test Transaction",
            "image": "https://media.istockphoto.com/id/1226386279/vector/vector-double-line-alternative-logo-letter-c.jpg?s=612x612&w=0&k=20&c=vstn9zTeMFjee-WY5G1KYX2expZMLpltpiDZkEgx1oI=",
            "order_id": order.id, //This is a sample Order ID. Pass the `id` obtained in the response of Step 1
            "handler": function (response){
                verifyPayment(response,order)
            },
            "prefill": {
                "name": "Gaurav Kumar",
                "email": "gaurav.kumar@example.com",
                "contact": "9000090000"
            },
            "notes": {
                "address": "Razorpay Corporate Office"
            },
            "theme": {
                "color": "#3399cc"
            }
        };
        var rzp1 = new Razorpay(options);
        rzp1.open()

        const verifyPayment = async(payment,order) => {
            const response = await verifyPaymentUser({payment,order,paymentData})
            if(response.success){
                navigate('/payment-success',{state:{paymentData}})
            }else{
                alert('payment failed')
            }
        }
    }

  return (
    <Fragment>
        <div className='paymentMainDiv'>
            <div className='contactDetailsDiv'>

                <div className='contactHeaderDiv'>
                    <h1 className='text-2xl font-bold'>Contact Details</h1>
                </div>
                
                
                <form onSubmit={handleSubmit}>
                <div className='contactFormDiv'>
                    <div className='flex '>
                    
                        <div >
                            <input className='formInputDiv' placeholder='Email' name='email' value={email} onChange={(e)=>setEmail(e.target.value)} required></input>
                        </div>
                        <div >
                            <input className='formInputDiv' placeholder='Phone' name='phone' value={phone} onChange={(e)=>setPhone(e.target.value)} required></input>
                        </div>
                    
                    </div>
                
                </div>

                <div className='flex'>

                <div className='paymentDiv mx-20 my-5'>
                    <div className=' mb-7'>
                        <h1 className='text-2xl font-bold'>Payment Options</h1>
                    </div>
                    <div className='flex' onClick={()=>handleRadio(checked)}>
                        <input type='radio' className='my-2 mr-2 mx-2' checked={checked} name='razorpay' value={payment} onClick={(e)=>setPayment(e.target.value)}></input>
                        <img alt='razorpayLogo' className='w-1/2' src='data:image/png;base64,'></img>
                    </div>
                    <div className='mt-10 -mx-2'>
                        <button className='submitButtonContact font-bold ' type='submit'>Proceed to payment</button>
                    </div>
                </div>
                
                <div className='bookingStatusDiv'>
                    <div className='bookingStatusInsideDiv'>
                        <div className="headerDiv">
                            <h1>Booking Details</h1>
                            <div className='flex flex-col'>
                                <h1 className='text-center text-xl font-normal'>{selectedSeats.length}</h1>
                                <h1 className='font-normal text-xl'>Tickets</h1>
                            </div>
                        </div>

                        <div className='ticketDetailsDiv'>
                            <h1 className='text-xl font-bold mb-4'>{showData.moviename}</h1>
                            <h1 >Seat No :- {selectedSeats.map((seat)=>(
                                seat.id + ','
                            ))} </h1>
                            <h1>Date :- {showDate}</h1>
                            <h1>Time :- {showData.showtime}</h1>
                            <h1>Theater :- {showData.theatername}</h1>
                        </div>

                        <hr className='border-slate-900'/>

                        <div className='priceDetailsDiv'>
                            <div className='flex justify-between'>
                                <h1 className=' font-medium text-lg'>Sub Total</h1>
                                <h1 className='font-medium'>{selectedSeats.length * showData.ticketprice}</h1>
                            </div>

                            <div className='otherDetails'>

                                <div className='flex justify-between'>
                                <h1>Convenience fee</h1>
                                <h1>{convenienceFee}</h1>
                                </div>

                                <div className='flex justify-between'>
                                <h1>Discount</h1>
                                <h1>{discount}</h1>
                                </div>

                               
                            </div>

                            <div className='flex justify-between'>
                                    <h1 className='font-medium mt-3 text-lg'>Total</h1>
                                    <h1 className='mt-3 font-semibold'>{total}/-</h1>
                            </div>

                        </div>


                    </div>

                </div>

                

                </div>
                </form>
                <div className='w-full h-20'>

                </div>
            </div>
        </div>
    </Fragment>
  )
}

export default PaymentSection
